...
def make_ui():
    return Harness(App(full=False, size="800x300", style=Style(fg="white", bg="black")))
...
def _render_and_click(ui, toast, index):
    """Compose once (populates toast._bounds), then dispatch a real MouseClick
...
def test_clicking_the_message_row_does_nothing():
    ...
    ui.click((left + 2, top + 1))  # message row, not buttons
    assert calls == []
    assert t in app._toasts

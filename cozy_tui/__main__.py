"""Interactive cozy_tui showcase — run it with ``python -m cozy_tui``.
...
def page_welcome(app, box):
    box.add(Label(2, 1, "Welcome to the cozy_tui demo!", ACCENT))
    box.add(Label(2, 3, "cozy_tui is a lightweight, cross-platform Python TUI library."))
    box.add(Label(2, 4, "Every frame is drawn into a cell grid and diffed to the terminal."))
...
def page_inputs(app, box):
    ...
    out = Label(2, 11, "", OK)
    box.add(Button(2, 10, "Greet").on_click(
...
def page_selection(app, box):
    ...
    box.add(ListView(2, 2, [
        ListItem("Python", "py"), ListItem("Rust", "rs"),
        ListItem("Go", "go"), ListItem("Zig", "zig"),
    ], height=4))
    box.add(Label(2, 7, "Theme (Dropdown):", ACCENT))
    box.add(Dropdown(2, 8, [
        ListItem("Dark", "dark"), ListItem("Light", "light"),
        ListItem("Solarized", "sol"),
    ], placeholder="choose…"))

    box.add(Label(28, 1, "Toppings (CheckList):", ACCENT))
    box.add(CheckList(28, 2, [
        CheckItem("Cheese"), CheckItem("Mushroom", checked=True), CheckItem("Olives"),
        CheckItem("Peppers"),
    ], height=4))
...
def page_overlays(app, box):
    ...
    def open_dialog(_b):
        ...
        dlg.add(Button(13, 3, "OK", style=Style(fg="white", bg="blue")).on_click(
            lambda b: (setattr(info, "text", "Dialog confirmed ✔"), app.close_overlay(dlg))))
        app.open_overlay(dlg, close_on_click_outside=True)

    box.add(Button(2, 3, "Open dialog").on_click(open_dialog))
    box.add(Button(16, 3, "Prompt…").on_click(
        lambda b: app.prompt("Type something:",
                             on_submit=lambda t: setattr(info, "text", f"You typed: {t}"))))
    box.add(info)


def page_about(app, box):
    box.add(Label(2, 1, "cozy_tui", ACCENT))
    ...
    box.add(Label(2, 11, f"Made with cozy_tui {_VERSION} 💜", MUTED))
...
def main() -> None:
    ...
    header = Box(0, 0, "10x10", title="✨ cozy_tui", border="rounded")
    glow = GlowAnimation(color_template="blue", speed=0.08)
    header.add(AnimatedLabel(
        1, 1, f"  cozy_tui {_VERSION}  —  a cozy, cross-platform terminal UI toolkit",
        animation=glow,
    ))
    app.dock(header, "top")
    ...
    def show(page):
        ...
        hint.text = (f"  {page}     ↑/↓ switch page    Tab: into page    "

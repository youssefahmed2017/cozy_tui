...
Planned design work for TermQuarium that's bigger than a changelog entry.
Each section is a design, not a promise of a ship date. TermQuarium is
developed and tested inside the [cozy_tui](https://github.com/youssefahmed2017/cozy_tui)
monorepo (as `examples/aquarium/`) and mirrored here; file paths below are
relative to this repo's own layout (`aquarium.py`, `termquarium/`, at the
root) unless noted.
...
`CHANGELOG.md` in the cozy_tui monorepo). Combination flavor text (step 3) is
in too, as a small, hand-picked table in `inspectors.py` (not every one of
the ~50 possible personality+trait/trait+trait pairs — just the four
`updates.md` actually gave text for). Keen Explorer's "visits new areas
first" pitch became "more eager to go explore the Forest"
(`KEEN_EXPLORER_FOREST_CHANCE_MULT`), since the Forest is the only other
area that actually exists to visit.

**Source:** `updates.md`, "🧬 Personality System 2.0" and the sections
following it (new personalities, personality interactions, personality
growth) — a design brainstorm, not yet built at the time. This section turns
that brainstorm into a plan grounded in the code as it exists.
...
(`termquarium/constants.py:184`) is a fixed tuple; `random_personality()`
(`termquarium/relationships.py:45`) rolls exactly **one**, uniformly, once
at birth, stored as a plain `self.personality: str`
(`termquarium/fish.py:196`). It's then read via `self.personality == "X"`
...
(`"personality": f.personality`, `aquarium.py:1173`). The development test
...
2. **Traits are earned, not rolled at birth** — deliberately unlike
   ...
   never rolled") rather than inventing a new one: `grant_trait()`, called at
   the moment something happens (eating a favorite treat, dreaming, a close
   shark scare, …), logged through the existing `memory_log`
...
3. **Combination flavor text is cosmetic, added last.** Once two or more
   ...
   "dreams about places it has never visited") live in `inspectors.py` as
   flavor, matching this codebase's existing "flavor, not optimization"
   thread (Treats/Axolotl/Random Events) — no gameplay effect depends on a
   specific *combination*, only on which individual tags are present.
4. **Save compatibility:** `traits` is a new, optional save key. A save
   without one (every save from before this) loads as `traits=frozenset()` —
   ...
   `aquarium.py:1298-1302`). `personality` itself doesn't move or change
...
5. **Cheat Console support**, matching `console.py`'s existing structural
   ...
   command calling the same real code path, for testing without waiting on
   the natural trigger.
...
without touching the steering-priority chain, `_claim_home()`, or any
existing personality-keyed test.

## Coral Valley (a second biome)
...
a scratch conversation, and so whoever eventually builds it starts from the
right hook points instead of re-deriving them.

**Source:** an internal design conversation's "First Visit Playthrough" — a
two-day narrative walkthrough of a new area, discovered via a Shop-style
unlock, built around discrete named places (Coral Gardens, Coral Castle and
its rooms, Coral Bridge, a Secret Coral Cave) and two fish (one exploring,
one settling in) accumulating shared memories. Its own stated design
philosophy, worth repeating verbatim since it's the thing that actually
matters:
...
just a place a bonded pair settles into. That contrast should stay
load-bearing in any real design, not get lost the moment implementation
starts.
...
N-scene system: `_enter_forest()`/`_leave_forest()` (`aquarium.py:1630` and `:1648`)
...
a single Shop row (`FOREST_UNLOCK_PRICE`, `termquarium/constants.py:1034`)
...
doesn't use it anywhere; the Forest toggle predates it or just never got
migrated. Adding a *second* biome is the natural forcing function to stop
hand-rolling this and move onto `Screen`, rather than copy-pasting
`in_forest`/`forest_widgets` a second time as
`in_coral_valley`/`coral_valley_widgets`.
...
- **Memories** ("Visited Coral Valley for the first time," "Slept in the
  ...
  `_log_memory()` (`aquarium.py:652`) — the same call already behind every
...
- **One fish explores, the other relaxes** (Steve investigates the Castle
  ...
  `aquarium.py:2654`, the weighting itself at `2688`) — this narrative would
...
- **"Found a favorite place"** already exists as `favorite_decoration`
  (`termquarium/fish.py:215`, rolled once at birth) — Coral Valley
  introducing its own favorite *location* is either a second,
  Coral-Valley-scoped instance of that same idea, or (cleaner) a
...
- **No danger system needed.** Deliberately: Coral Valley's whole point is
  ...
  (`aquarium.py:3518`) should have no Coral Valley equivalent — resist the
...
1. **Multi-room navigation: yes, a real nested structure**, not one flat
   scene. Three levels — the structural shape below is decided; exact
   layouts/art are **not** (an early illustrative sketch, not a mockup to
   build literally):
...
4. **Yes — biome residency should survive Save/Load. Day/night phase and
   ...
   before this, so a save/load round trip silently reset the world to
   midday with no one dreaming. Fixed via a persisted `day_fraction` (the
   same 0..1 value `_update_environment()` already computes, reapplied to
   `session_start` on load) and a fully self-contained per-fish `dream`
...
**Bonus:** Coral Valley is Axolotls' favorite place — they should be the
species most likely to visit it, stay there (a natural fit for
`favorite_decoration`-style "favorite location" once that generalizes, per
the note above), and explore it once there.

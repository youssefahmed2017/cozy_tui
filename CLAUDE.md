...
### Overlays / z-layer
`app.open_overlay(widget)` pushes onto `_overlays` (drawn above the base UI). **Modal** overlays capture all keyboard/mouse input and (with `dim`) grey the background via `_apply_backdrop()`; non-modal ones are purely visual (tooltips, `Toast`). `app.prompt(...)`/`app.confirm(...)`/`app.pick_file(...)` are modal dialogs built on this (`PromptDialog`/`ConfirmDialog`/`FilePicker`); `ThemePalette`/`CommandPalette` (bound to Ctrl+T/Ctrl+P by default, see "Theming" below) are the same pattern. Every one of these is a **single self-contained widget** — its own text buffer, its own Up/Down/Enter navigation, its own `draw()` — rather than composed from a real `Input` + `ListView`: a modal routes *every* key to one focused widget with no bubbling, so splitting the text-entry half and the list-navigation half across two child widgets would need a hand-written wrapper deciding which key goes where anyway, for no less code than handling it directly. Overlays position in screen space (no scroll), re-centered each frame if `center=True`. One quirk worth knowing: inside `_dispatch_input`'s modal branch, **F12 always reaches `toggle_debug_pane()`** rather than being treated as a second "close this modal" key alongside Esc — otherwise F12 could never open the debug pane while any of the above happens to be open.

### Theming (`theme.py`)
A `Theme` bundles the colors the shared visual language draws from: the base `App.style`, `accent`, `success`/`warning`/`error`/`info` (what `Toast` reads its color from), `muted`, and `selection_fg`/`selection_bg` (what `style.py`'s `selection_style()` — shared by `ListView`, `RadioSet`, `CheckList`, `Table`, `Tree`, `Dropdown`, `Checkbox`, `RightClickMenu`, `Slider`, `MenuBar` — builds its focused-row highlight from). Built from a built-in `mode` (`Theme.MODES`, `_presets.py`; over 20 and growing) and/or per-role overrides. There is exactly **one process-wide active theme** (module-level global state in `theme.py`): `get_theme()`/`set_theme(theme)`/`theme.activate()`. `App()` built with no explicit `style=` picks up the active theme's `style` (a fresh copy, not the same object — mutating one app's `.style` must never leak into another's). Widgets do **not** retroactively repaint on a later theme switch — colors are resolved once, at construction/draw time, like the rest of this library's styling — except `selection_style()`, which is re-read from the active theme every draw. `App.open_theme_palette()` (Ctrl+T) is a searchable `ThemePalette` over `Theme.MODES`; `App.cycle_theme()` is a plain next-mode advance, not bound by default. `App.open_command_palette()` (Ctrl+P) is the same `CommandPalette` pattern generalized to arbitrary actions, fed by `App.register_command(name, callback, description=)` — `App` itself registers "Quit"/"Change Theme"/"Keys" (and "Toggle Debug Pane" when `debug=True`) this way, so overriding one of those names replaces it.
